  ...
  Object.entries(languageCounts).forEach(([language, count]) => {
    ...
    if (mapping) {
      ...
      if (!existingSkill) {
        ...
        existingSkill.repositories = [...new Set([...existingSkill.repositories, ...repos])];
...
function main() {
  ...
  if (!projects) {
    console.error('❌ Could not read projects.json. Run sync-github-data.js first.');
    process.exit(1);
  }
  ...
  if (organizations && Array.isArray(organizations)) {
    ...
    console.log(`✓ Loaded ${organizations.length} organizations with ${orgRepos.length} repositories`);
  }
  ...
  const totalSkills = Object.values(skills).reduce((sum, cat) => sum + cat.length, 0);
  ...
  Object.entries(skills).forEach(([category, categorySkills]) => {
    if (categorySkills.length > 0) {
      ...
      categorySkills.forEach(skill => {
        console.log(`    - ${skill.name} (${skill.level}) - ${skill.count} repos`);
      });
    }
  });
