...
const syncedSkillsData: SyncedSkillCategory = {\n`;
  ...
  ['frontend', 'backend', 'database', 'mobile', 'cloud', 'tools'].forEach(category => {
...
function main() {
  ...
  if (projects || organizations) {
    ...
    const totalProjects = (projects?.length || 0) + 
      (organizations?.reduce((sum, org) => sum + (org.repositories?.length || 0), 0) || 0);
  ...
  if (skillsData) {
    ...
    if (skillsTS) {
      ...
      const totalSkills = Object.values(skillsData.skills)
        .reduce((sum, cat) => sum + cat.length, 0);
      console.log(`  - ${totalSkills} skills from ${skillsData.totalRepositories} repositories`);
      filesGenerated++;
    }
  }
  ...
  if (filesGenerated === 0) {
    ...
    console.log('\n' + '='.repeat(50));
    console.log(`✓ Successfully generated ${filesGenerated} TypeScript file(s)`);
    console.log('='.repeat(50));
    console.log('\nNext steps:');
